  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-700">
      <div>
        ...
        <p className="text-warm-500 dark:text-warm-400 mt-1">Directorio de usuarios, roles de administración y gestión de privacidad ARCO.</p>
      </div>

      <ClientsTableClient users={JSON.parse(JSON.stringify(users))} />
      
      <hr className="border-warm-200 dark:border-warm-800" />
      
      <ArcoTableClient initialRequests={JSON.parse(JSON.stringify(arcoRequests))} />
    </div>
  );
}
